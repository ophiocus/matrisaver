...
fn main() {
    ...
    if list_adapters {
        for adapter in runtime.adapter_snapshots() {
            println!(
                "ADAPTER host=macos name={} backend={} device_type={}",
    ...
    run_runtime_benchmark("macos", &mut runtime, benchmark_frames, width, height);

    println!(
        "macOS host stub ready. Next step: bridge to ScreenSaver lifecycle and bundle packaging."
    );
}
...
mod tests {
    ...
    fn parses_explicit_gpu_selection_flags() {
        let args = vec![
            "app".to_string(),
            "--wgpu-backend=metal".to_string(),
            "--wgpu-adapter-name".to_string(),
            "apple".to_string(),
        ];
        let selection = parse_gpu_selection(&args);
        assert_eq!(selection.backend.as_deref(), Some("metal"));
        assert_eq!(selection.adapter_name.as_deref(), Some("apple"));
    }
}
